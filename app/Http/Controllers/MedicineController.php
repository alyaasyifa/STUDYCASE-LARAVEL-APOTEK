...
class MedicineController extends Controller
{
    ...
    public function store(Request $request)
    {
        //
        $request->validate([
            'name' => 'required|min:3', // ngambil name dari form
            'type' => 'required',
            'price' => 'required|numeric',
            'stock' => 'required|numeric',
        ]);

        Medicine::create([ //kalo udah memenuhi maka panggil model, dan memanggil langsung create
            'name' => $request->name, // ngambil request dari name, ngambil name dari kolom
            'type' => $request->type,
            'price' => $request->price,
            'stock' => $request->stock,
        ]);

        return redirect()->back()->with('success', 'Berhasil menambahkan data obat!'); //akan di arakan ke back, dan kl berhsil makan akan menampilkan alert
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Medicine::where('id', $id)->update([ // harus memakai where agar tidak semua ke update
            'name' => $request->name,
            'type' => $request->type,
            'price' => $request->price,
        ]);
